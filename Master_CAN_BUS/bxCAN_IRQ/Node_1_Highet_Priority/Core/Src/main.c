...
void CAN_TX(uint32_t ID, uint8_t DLC ,uint8_t * payload,uint8_t PollingEn)
{
	...
	if(no_free_txMailBox)
	{
		...
		//     (++) HAL_CAN_IsTxMessagePending() to check if a message is pendingin a Tx mailbox.
		if(PollingEn)
			{
...
void CAN_RX_Filter_Init(uint16_t STD_Filter_ID ,uint16_t STD_Filter_Mask)
{
	...
	CAN_FilterTypeDef sFilterConfg;
	sFilterConfg.FilterActivation=CAN_FILTER_ENABLE;
	sFilterConfg.FilterBank=0;
	sFilterConfg.FilterFIFOAssignment=CAN_FILTER_FIFO0;
	sFilterConfg.FilterIdHigh=STD_Filter_ID<<5;
	sFilterConfg.FilterIdLow=0x0000;
	sFilterConfg.FilterMaskIdHigh=STD_Filter_Mask<<5;
	sFilterConfg.FilterMaskIdLow=0x0000;

	sFilterConfg.FilterMode=CAN_FILTERMODE_IDMASK;
	sFilterConfg.FilterScale=CAN_FILTERSCALE_32BIT;

	if(HAL_CAN_ConfigFilter(&hcan,&sFilterConfg)!=HAL_OK)
	{
		Error_Handler();
	}
...
int main(void)
{
  ...
  {
    HAL_Delay(1000);
    CAN_TX(0x30, 1,TX_Data,0);
    TX_Data[0] ^= 1;    // 1 >>ACC_ON   0>> ACC_OFF
  }
  /* USER CODE END 3 */
}
